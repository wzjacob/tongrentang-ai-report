...
import { AlertTriangle, Cpu, DatabaseZap, ShieldCheck, Sparkles, Stethoscope, Workflow } from "lucide-react";
...
const deepDiveData = {
  ...
  leftPanel: {
    ...
    items: [
      {
        name: "图谱基座数据",
        desc: "引入 Neo4j 图谱，保留图谱基础数据和标签体系，重建关联关系；对接入口重新调试，要求中台具备图谱数据库扩展能力。",
        color: "blue" as const,
      },
  ...
  rightPanel: {
    tech: {
      title: "实现路径：图谱检索与规则校验",
      desc: "基于“药材-病症-禁忌”关系网进行多跳检索，并叠加规则校验，显著降低误答与合规风险。",
      compactDesc: "以图谱检索 + 规则校验提升问答准确性与合规性。",
    },
    features: [
      {
        title: "多跳逻辑精准问答",
  ...
  delivery: {
    title: "流程关联与场景落地",
    links: [
      "业务中枢 -> 数据填入层：对说明书、禁忌、门店问答进行向量化与图谱索引。",
      "业务中枢 -> 管控层：按岗位权限控制“可答范围”，敏感问法自动拦截并留痕审计。",
      "业务中枢 -> 计算资源层：根据问题复杂度动态路由资源，保障时效与稳定性。",
      "业务中枢 -> 数据底座层：统一调用 Neo4j 图谱与向量库，输出可追溯引用答案。",
    ],
    scenes: [
      ...
      <div className="mx-auto max-w-6xl">
          ...
          <section className="rounded-3xl border border-[#e5e7eb] bg-white p-5 shadow-[0_10px_24px_rgba(15,23,42,0.05)] md:p-6">
            ...
            <div className="mt-4 space-y-3">
              ...
              <article className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4">
                <p className="text-sm font-semibold text-amber-800">药品解析数据</p>
        ...
        <section className="mt-5 rounded-3xl border border-[#e5e7eb] bg-white p-5 shadow-[0_10px_24px_rgba(15,23,42,0.05)] md:p-6">
          ...
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#b91c1c]">流程关联图</p>
              <div className="mt-2 rounded-xl border border-rose-100 bg-rose-50/40 p-3">
                <div className="grid gap-3 md:grid-cols-[160px_1fr] md:items-start">
                  <div className="rounded-lg border border-rose-200 bg-white px-3 py-3 text-center">
                    <p className="text-xs font-semibold text-rose-700">业务中枢</p>
                    <p className="mt-1 text-[11px] text-[#6b7280]">统一编排与路由</p>
                  </div>
                  <div className="space-y-2 border-l-2 border-rose-200 pl-3">
                    {deepDiveData.delivery.links.map((item, idx) => {
                      ...
                      const summary = showFullVersion ? desc : `${desc.slice(0, 28)}...`;
                      return (
                        <article key={item} className="relative rounded-lg border border-rose-200 bg-white/95 p-2.5">
                          <span className="absolute -left-[14px] top-3 h-2.5 w-2.5 rounded-full border border-rose-200 bg-rose-300" />
                          <div className="flex items-center gap-1.5">
                            <span className="inline-flex h-6 w-6 items-center justify-center rounded-md bg-rose-50 text-[#b91c1c]">
                              <Icon className="h-3.5 w-3.5" />
                            </span>
                            <p className="text-xs font-semibold text-rose-700">{to}</p>
                          </div>
                          <p className="mt-1 text-[11px] leading-relaxed text-[#4b5563]">{summary}</p>
                        </article>
                      );
                    })}
                  </div>
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#b91c1c]">业务场景</p>
